...
<div class="panel" style="max-width:560px">
  ...
  <?php
    $hasAnyTrustInfo = $selected
        && (! empty($selected['trust_name']) || ! empty($selected['trust_address']) || ! empty($selected['trust_pincode'])
            || ! empty($selected['trust_pan']) || ! empty($selected['trust_phone']) || ! empty($selected['trust_email']));
  ?>
  <?php if ($selectedId && ! $selected): ?>
    <p class="panel-note contact-info">Prant not found.</p>
  <?php elseif ($selected): ?>
    <?php if (! $hasAnyTrustInfo): ?>
    ...
    <?php else: ?>
      <div class="contact-info">
        ...
        <?php endif; ?>
        <?php if (! empty($selected['trust_phone'])): ?>
          <div class="contact-field">
            <div class="contact-label">Phone</div>
            <div class="contact-value num"><a href="tel:<?= esc($selected['trust_phone'], 'attr') ?>"><?= esc($selected['trust_phone']) ?></a></div>
          </div>
        <?php endif; ?>
        <?php if (! empty($selected['trust_email'])): ?>
          <div class="contact-field">
            <div class="contact-label">Email</div>
            <div class="contact-value"><a href="mailto:<?= esc($selected['trust_email'], 'attr') ?>"><?= esc($selected['trust_email']) ?></a></div>
          </div>
        <?php endif; ?>

...
<?php $configured = count(array_filter($prants, fn ($p) => ! empty($p['trust_name']))); ?>
...
<div class="panel" style="background:var(--surface-alt)">
  <div class="panel-head">
    <div>
      ...
      <div class="panel-note" style="margin-top:3px">Upload a spreadsheet to set the Trust name, address, PIN and PAN for many Prants at once — rows are matched to an existing Prant by name, never create a new one.</div>
    </div>
...
<div class="panel">
  ...
  <div class="table-wrap">
    <table>
      <thead><tr><th>Prant</th><th>Registered Trust Name</th><th>PIN Code</th><th>PAN</th><th>Status</th><th></th></tr></thead>
      <tbody>
      <?php foreach ($prants as $p): ?>
        <tr>
          ...
          <td><span class="pill <?= status_pill_class(empty($p['trust_name']) ? 'pending' : 'configured') ?>"><?= empty($p['trust_name']) ? 'Pending' : 'Configured' ?></span></td>
          <td><a class="btn btn-ghost btn-sm" href="?edit=<?= $p['id'] ?>"><?= empty($p['trust_name']) ? 'Configure' : 'Edit' ?></a></td>
        </tr>
      <?php endforeach; ?>
      </tbody>
    </table>

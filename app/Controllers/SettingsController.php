...
class SettingsController extends BaseController
{
    ...
    public function downloadTrustTemplate()
    {
        ...
        $sheet->fromArray(['Prant', 'Registered Trust Name', 'Address', 'PIN Code', 'PAN'], null, 'A1');
        $sheet->getStyle('A1:E1')->getFont()->setBold(true);
        $rows = array_map(static fn ($p) => [
            $p['name'], $p['trust_name'] ?? '', $p['trust_address'] ?? '', $p['trust_pincode'] ?? '', $p['trust_pan'] ?? '',
        ], $prants);
        ...
        $notes->fromArray([
            ...
            ['PAN must be in the standard format, e.g. AAATV1234C. PIN Code must be 6 digits.'],
        ], null, 'A1');
